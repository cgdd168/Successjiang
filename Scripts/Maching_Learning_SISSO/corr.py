...
features_corr=np.round(df.corr(),1).reset_index()
mydata=pd.melt(features_corr,id_vars='index',var_name='var',value_name='value')
...
base_plot=(ggplot(mydata, aes(x ='index', y ='var', fill = 'value',size='AbsValue')) +  
  ...
  scale_size_area(max_size=44, guide=False) +
  scale_fill_cmap(name ='RdYlBu_r')+
  coord_equal()+
    theme(dpi=600,figure_size=(16,16)))
print(base_plot)
